...
const DownloadHero = () => {
    return <div>
        ...
        <div className="px-4 my-12 mx-5">
            <p className="text-xl font-semibold">INSTALLATION AND ACTIVATION INSTRUCTIONS:</p>
            <br></br>
            <p className="text-lg">The .exe installer is a per-user installer and does not require any admin install privileges. Don't "Run as Admin" or it may not appear on the current
                user's profile. <br></br>
                The .msi installer is a per-machine installer and will require admin install privileges.<br></br>
                Both installers have a -quiet switch for silent installation by deployment software.<br></br>
                Single-User Licenses:<br></br>
                Purchase<br></br>
                Purchase the quantity of licenses needed for your team.<br></br>
                All licenses purchased are attributed to the company email domain name (unless using a personal email).<br></br>
                Each single user license authorizes you to register two computers for the same user. A unique registration code must be requested from each
                computer. A separate license should be purchased for each user that will use the tools. <br></br>
                Notify Users<br></br>
                You can immediately notify users that they can install and activate the software.<br></br>
                Each user license request will be counted toward the total number purchased for the company on a first come, first served basis. If you would like to
                submit a list of authorized users via email to control who can request activations, that can be accommodated as well. <br></br>
                Click Activate<br></br>
                Your user(s) can immediately install the tools and click the Licensing/Activation button on the RF Tools ribbon in Revit. Use the 'Request individual
                user registration key' option to generate and send in an activation request code. Check your spam folder or send a follow up email if your registration
                key is not received within 24 hours of sending your activation request email.<br></br>
                As long as the user email address domain matches the purchasing company email address domain, the activation will be attributed to the purchase</p>

            <p className="mt-5 text-lg">Note: All licenses are annual and apply only to the specific Revit version purchased. Each version of Autodesk Revit requires its own compatible version
                of the HamstrUK Plug-In. Licenses and installations are sold separately for each Revit version.</p>
        </div>
