...
const HomeDashboard = () => {
  ...
  const getEditData = (id) => {
    ...
    if (currentRecord == 1) {
      ...
      try {
        api.get(`${import.meta.env.VITE_API_URL}/api/xl/${id}`).then((res) => {
          setFirstInputValue(res.data.data.description);
          setSecondInputValue(res.data.data.link);
        });
      } catch (err) {
  ...
  const addFoundation = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/foundation`, data);
  ...
  const addFloor = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/floor`, data);
      toast.success("Record Added Succesfully");
    } catch (err) {
      toast.error(err);
    }
  ...
  const addBeams = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/beams`, data);
      toast.success("Record Added Succesfully");
    } catch (err) {
      toast.error(err);
    }
  ...
  const addStructural = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/structural`, data);
      toast.success("Record Added Succesfully");
    } catch (err) {
      toast.error(err);
    }
  ...
  const addInformation = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/information`, data);
      toast.success("Record Added Succesfully");
    } catch (err) {
      toast.error(err);
    }
  ...
  const addXL = (data) => {
    try {
      api.post(`${import.meta.env.VITE_API_URL}/api/xl`, data);
      toast.success("Record Added Succesfully");
    } catch (err) {
      toast.error(err);
    }
  ...
  const updateFoundation = (data) => {
    ...
    try {
      api.put(
        `${import.meta.env.VITE_API_URL}/api/foundation/${editId}`,
        updatedData,
      );
  ...
  const updateFloor = (data) => {
    ...
    try {
      api.put(
        `${import.meta.env.VITE_API_URL}/api/floor/${editId}`,
        updatedData,
      );
      toast.success("Record Updated Succesfully");
    } catch (err) {
  ...
  const updateBeams = (data) => {
    ...
    try {
      api.put(
        `${import.meta.env.VITE_API_URL}/api/beams/${editId}`,
        updatedData,
      );
      toast.success("Record Updated Succesfully");
    } catch (err) {
  ...
  const updateStructural = (data) => {
    ...
    try {
      api.put(
        `${import.meta.env.VITE_API_URL}/api/structural/${editId}`,
        updatedData,
      );
      toast.success("Record Updated Succesfully");
    } catch (err) {
  ...
  const updateInformation = (data) => {
    ...
    try {
      api.put(
        `${import.meta.env.VITE_API_URL}/api/information/${editId}`,
        updatedData,
      );
      toast.success("Record Updated Succesfully");
    } catch (err) {
  ...
  const updateXL = (data) => {
    ...
    try {
      api.put(`${import.meta.env.VITE_API_URL}/api/xl/${editId}`, updatedData);
      toast.success("Record Updated Succesfully");
    } catch (err) {
      toast.error(err);
    }
          ...
          <div className="flex flex-col justify-center items-center gap-4 mt-10">
            {foundation.map((elem, idx) => (
              <AdminList
                ...
                secondDisc={elem.link}
                handleEditRecord={hanldeEditRecord}
                current={currentRecord}
                setFunc={setFoundation}
              />
            ))}
          </div>
        )}
      ...
      <>
        {addRecord == true && currentRecord == 1 && (
          <DashboardForm
            ...
            placeholder2="Enter Youtube Link"
            buttonName="Add Record"
            addFormData={addFoundation}
          />
        )}
